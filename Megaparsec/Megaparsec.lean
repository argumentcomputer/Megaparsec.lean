...
inductive ErrorFancy where
| fail (msg: String)
| indent (ord: Ordering) (fromPos: Pos) (uptoPos: Pos)
| custom (e: E)

inductive ParseError (s : Stream S) where
| trivial (offset: Nat) (unexpected: ErrorItem s.Token) (expected: List (ErrorItem s.Token))
| fancy (offset: Nat) (expected : List ErrorFancy)

structure State (S E : Type) (s : Stream S) where
  stateInput       : S
  stateOffset      : Nat
  statePosState    : PosState S
  stateParseErrors : List (ParseError s)
...
def withHints {M : Type u → Type v} {stream : Stream S}
                -- | Hints to use
              (ps' : Hints (stream.Token))
                -- | Continuation to influence
              (c : ParseError stream → State S E stream → M B)
                -- | First argument of resulting continuation
              (e : ParseError stream) : State S E stream → M B :=
  match e with
    | ParseError.trivial pos us ps => c $ ParseError.trivial pos us (ps ++ ps')
    | _ => c e


def pBind [m: Monad M] (p : @ParsecT S M E s m A) (k : A → @ParsecT S M E s m B) : @ParsecT S M E s m B :=
...
instance [m: Monad M] : Monad (ParsecT E s m) where
  bind := pBind
...
class MonadParsec (E S : Type) [Monad M] [Alternative M] (stream : Stream S) where
  ...
  parseError (A: Type) (err: ParseError stream): M A
  ...
  withRecovery (A: Type) (phi: ParseError stream → M A) (parser: M A): M A
  -- | Observes 'ParseError's as they happen, without backtracking.
  observing (A: Type) (parser: M A): M (Either (ParseError stream) A)
